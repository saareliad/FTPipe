        ...
        for d in depth:
            ...
            if net.__name__.find("inception") != -1:
                pipeline, graph, _ = distribute_model(model, torch.zeros(
                    4, 3, 299, 299).to(device), device_list=["cuda", "cpu"], num_iter=4, max_depth=d, basic_blocks=None)

            elif net.__name__.find("GoogLeNet") != -1:
                pipeline, graph, _ = distribute_model(model, torch.zeros(
                    4, 3, 32, 32).to(device), device_list=["cuda", "cpu"], num_iter=4, max_depth=d, basic_blocks=None)
                ...
                pipeline, graph, _ = distribute_model(model, torch.zeros(
                    4, 3, 224, 224).to(device), device_list=["cuda", "cpu"], num_iter=4, max_depth=d, basic_blocks=None)

            filename = f"{net.__name__} attempted {2} partitions at depth {d}"
            curr_dir = os.path.dirname(os.path.realpath(__file__))
            out_dir = f"{curr_dir}\\unoptimized"
            graph.save(directory=out_dir, file_name=filename,
                       show_buffs_params=False, show_weights=False)
