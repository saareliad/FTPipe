...
def get_config_cpu_only():
    ...
    stage3 = config.add_stage(dummy.Stage3, torch.optim.AdamW).add_input(
        "t1", (200, 100)).add_output("output0", (200, 100)).add_output("output1", (200, 100)).add_devices('cpu', 'cpu')
...
def test_realize_cpu_only():
    config = get_config_cpu_only()
    model = dummy.Dummy()
    depth = config.depth
    blocks = config.basic_blocks
    layers = layerDict(model, depth=depth, basic_blocks=blocks)
