...
class SympyAdam(SympyPredictingOptimizer):
    ...
    #     exp_avg_sq.mul_(beta2).addcmul_(1 - beta2, grad, grad)
        ...
        denom = (sympy.sqrt(self.exp_avg_sq) /
                 (sympy.sqrt(bias_correction2)) + self.eps)
    ...
    def prediction(self, nsteps):
        ...
        for i in range(1, nsteps+1):
            ...
            momentum_coeff += ((sympy.sqrt(bias_correction2)) /
                               bias_correction1) * (beta1 ** i)
...
class NormalSympyAdam(SympyAdam):
    ...
    def prediction(self, nsteps):
        ...
        for i in range(1, nsteps+1):
            ...
            denom = (sympy.sqrt(exp_avg_sq) /
                     (sympy.sqrt(bias_correction2)) + eps)
...
def run_sim(nsteps, optimizer_cls: SympyPredictingOptimizer = SympySGD, simplify=True):
    ...
    gaps = [calc_gap(i, j, simplify=simplify)
...
def run_and_display_sim(nsteps, optimizer_cls=SympySGD, displayer=pprint, simplify=True):
    theta_true, theta_preds, gaps = run_sim(
        nsteps, optimizer_cls=optimizer_cls, simplify=simplify)
