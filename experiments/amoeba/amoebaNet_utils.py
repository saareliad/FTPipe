...
class net(nn.Module):
    def __init__(self, input_shape, filter_size, hiddenstate_indices, operations
                 ...
                 , total_num_cells, used_hiddenstates, prev_output: torch.Tensor = None):
        super().__init__()
        self.p1 = [
            nn.ReLU(),
        ...
        if self.prev_output is not None:
            if input_shape[1] != self.prev_output.shape[1]:
                ...
                self.p2.append(nn.ReLU())
                self.p2.append(
                    nn.Conv2d(in_channels=self.prev_output.shape[1], out_channels=self.filter_size, kernel_size=1))
        ...
        for iteration in range(5):
            ...
            self.p5 = self.p5 + [
                opera(input_shape, filter_size, operation_left, stride, original_input_left
                      ...
                      self.track_shape[left_hiddenstate_idx]),
                opera(input_shape, filter_size, operation_right, stride, original_input_right
                      , drop_path_keep_prob, cell_num, total_training_steps, total_num_cells,
                      self.track_shape[right_hiddenstate_idx])
            ]
    ...
    def forward(self, x):
        ...
        if self.prev_output is not None:
            ...
            elif self.filter_size != self.prev_output.shape[1]:
        ...
        final_height = int(output1[-1].shape[2])
        final_num_filters = int(output1[-1].shape[1])
        assert len(self.used_hiddenstates) == len(output1)

        for idx, used_h in enumerate(self.used_hiddenstates):
            curr_height = int(output1[idx].shape[2])
            curr_num_filters = int(output1[idx].shape[1])
            # Determine if a reduction should be applied to make the number of
            # filters match.
            should_reduce = final_num_filters != curr_num_filters
            should_reduce = (final_height != curr_height) or should_reduce
            ...
            if should_reduce:
                ...
                if len(self.suc[idx]) > 1:
                    ...
                    for idx2, layer in enumerate(self.suc[idx][1]):
                        if idx2 == 0:
                            output6 = layer(output6)[:, :, 1:, 1:]
                        else:
                            output6 = layer(output6)
                    output1[idx] = torch.cat((output5, output6), dim=1)
                    output1[idx] = self.BN2[idx](output1[idx])
...
def _stacked_separable_conv(stride, operation, filter_size, track_shape):
    ...
    for layer_num in range(num_layers - 1):
        ...
        if layer_num == 0:
            layers.append(
                depthwise_separable_conv(track_shape[1], filter_size, kernel_size, depth_multiplier=1, stride=stride))
        else:
            layers.append(
                depthwise_separable_conv(filter_size, filter_size, kernel_size, depth_multiplier=1, stride=stride))
        layers.append(nn.BatchNorm2d(filter_size))

        track_shape[1] = filter_size
        track_shape[2] = int(((track_shape[2] - kernel_size) / stride) + 1)
        track_shape[3] = int(((track_shape[3] - kernel_size) / stride) + 1)
...
def _apply_conv_operation(operation, stride, filter_size, track_shape):
    ...
    if operation == '1x1':
        layers.append(nn.Conv2d(in_channels=track_shape[1], out_channels=filter_size, kernel_size=1))

        track_shape[1] = filter_size

    elif operation == '3x3':
        layers.append(nn.Conv2d(in_channels=track_shape[1], out_channels=filter_size, kernel_size=3, stride=stride))

        track_shape[1] = filter_size
        track_shape[2] = int(((track_shape[2] - 3) / stride) + 1)
        track_shape[3] = int(((track_shape[3] - 3) / stride) + 1)

    elif operation == '1x7_7x1':
        layers.append(
            nn.Conv2d(in_channels=track_shape[1], out_channels=filter_size, kernel_size=[1, 7], stride=[1, stride]))
        layers.append(nn.BatchNorm2d(filter_size))
        layers.append(nn.ReLU())
        layers.append(
            nn.Conv2d(in_channels=filter_size, out_channels=filter_size, kernel_size=[7, 1], stride=[stride, 1]))

        track_shape[1] = filter_size
        track_shape[3] = int(((track_shape[3] - 7) / stride) + 1)
        track_shape[2] = int(((track_shape[2] - 7) / stride) + 1)

    elif operation == '1x3_3x1':
        layers.append(
            nn.Conv2d(in_channels=track_shape[1], out_channels=filter_size, kernel_size=[1, 3], stride=[1, stride]))
        layers.append(nn.BatchNorm2d(filter_size))
        layers.append(nn.ReLU())
        layers.append(
            nn.Conv2d(in_channels=filter_size, out_channels=filter_size, kernel_size=[3, 1], stride=[stride, 1]))
        ...
        layers.append(
            nn.Conv2d(in_channels=track_shape[1], out_channels=filter_size, kernel_size=3, dilation=dilation_rate,
                      stride=stride))

        track_shape[1] = filter_size
        track_shape[3] = int(((track_shape[3] - dilation_rate * 2 - 1) / stride) + 1)
        track_shape[2] = int(((track_shape[2] - dilation_rate * 2 - 1) / stride) + 1)
...
def _pooling(stride, operation, track_shape):
    ...
    if pooling_type == 'avg':
        layer = nn.AvgPool2d(kernel_size=pooling_shape, stride=stride, padding=(pooling_shape - stride) // 2,
                             ceil_mode=True)
    elif pooling_type == 'max':
        layer = nn.MaxPool2d(kernel_size=pooling_shape, stride=stride, padding=(pooling_shape - stride) // 2,
                             ceil_mode=True)
...
class opera(nn.Module):
    def __init__(self, input_shape, filter_size, operation, stride, is_from_original_input
                 , drop_path_keep_prob, cell_num, drop_path_burn_in_steps, num_cells, track_shape):
        super().__init__()
        ...
        if 'separable' in operation:
            self.layers, self.track_shape = _stacked_separable_conv(stride, operation, filter_size, self.track_shape)
            ...
            if reduced_filter_size < 1:
                ...
                self.layers.append(nn.ReLU())
                self.layers.append(
                    nn.Conv2d(in_channels=self.track_shape[1], out_channels=reduced_filter_size, kernel_size=1))
                ...
                self.layers.append(nn.BatchNorm2d(filter_size))
                self.track_shape[1] = filter_size
            ...
            if stride > 1 or operation == '1x1' or (input_filters != filter_size):
                self.layers.append(nn.ReLU())
                self.layers.append(
                    nn.Conv2d(in_channels=self.track_shape[1], out_channels=filter_size, kernel_size=1, stride=stride))
                self.layers.append(nn.BatchNorm2d(filter_size))
                self.track_shape[1] = filter_size
                self.track_shape[2] = int(((self.track_shape[2] - 1) / stride) + 1)
                self.track_shape[3] = int(((self.track_shape[3] - 1) / stride) + 1)

        elif 'pool' in operation:
            l, _, self.track_shape = _pooling(stride, operation, self.track_shape)
            self.layers.append(l)
            if input_filters != filter_size:  # maybe channel dim is not right check later
                self.layers.append(
                    nn.Conv2d(in_channels=self.track_shape[1], out_channels=filter_size, kernel_size=1, stride=1))
                self.layers.append(nn.BatchNorm2d(filter_size))
                self.track_shape[1] = filter_size
        else:
    ...
    def forward(self, x):
        if 'separable' in self.operation or self.operation in ['dilated_3x3_rate_2', 'dilated_3x3_rate_4',
                                                               'dilated_3x3_rate_6', '3x3', '1x7_7x1',
                                                               '1x3_3x1'] or self.operation in ['none', '1x1']:
        ...
        if self.operation != 'none':
            x = _apply_drop_path(x, self.drop_path_keep_prob, self.cell_num, self.drop_path_burn_in_steps,
                                 self.num_cells, self.current_step)
        return x
...
def _apply_drop_path(x, drop_path_keep_prob, cell_num, drop_path_burn_in_steps, num_cells, current_step,
    ...
    if drop_path_keep_prob < 1.0:
        ...
        if drop_connect_version in ['v1', 'v3']:
            ...
            current_ratio = min(1.0, current_ratio)
