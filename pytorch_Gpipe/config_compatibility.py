...
from pytorch_Gpipe.pipeline.config import PipelineConfig
from collections import deque
from typing import List, Tuple, Set, Dict
import models.normal
import models.partitioned


def get_old_format(path: str, model: nn.Module):
    path = path.replace("/", ".")
    if path.endswith(".py"):
        path = path[:-3]
...
def get_new_format(path: str, model: nn.Module, sample: Tuple[Tensor, ...], batch_dim: int):
    path = path.replace("/", ".")
    if path.endswith(".py"):
        path = path[:-3]
    ...
    new_config = PipelineConfig(batch_dim, batch_size,
...
if __name__ == "__main__":
    model = models.normal.resnet50()
    old_to_old = get_old_format(
        "models/partitioned/legacy/resnet50_imagenet_p8.py", model)
    new_to_old = get_old_format("models.partitioned.resnet50.resnet50", model)

    sample = torch.randn(32, 3, 224, 224)
    batch_dim = 0
    new_to_new = get_new_format("models.partitioned.resnet50.resnet50",
                                model, sample, batch_dim)

    old_to_new = get_new_format(
        "models/partitioned/legacy/resnet50_imagenet_p8.py", model, sample, batch_dim)
