...
__all__ = ['partition_with_profiler', 'pipeline_using_profiler', 'pipeline_using_custom_weights',
...
def pipeline_using_profiler(model, *sample_batch, device_list=None, num_iter=4, max_depth=100, basic_blocks=None):
    ...
    return modified_model, graph


def pipeline_using_custom_weights(model, weights, *sample_batch, device_list=None, max_depth=100, basic_blocks=None):
    if device_list is None:
        if torch.cuda.is_available():
            device_list = list(range(torch.cuda.device_count()))
        else:
