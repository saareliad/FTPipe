# Hacking `torch.distributed.launch`
r"""
...
def main_with_args_kw(training_script, training_script_args, nnodes=1, node_rank=0, nproc_per_node=1):
    """ 
    "PyTorch distributed training launch 
                                        "helper utilty that will spawn up "
                                        "multiple distributed processes"
            
        Hacking `torch.distributed.launch` so it could be later used from python scripts without ugly bash calls.

        `training_script_args` could be a python dict.
        Requires that `training_script` won't have positional arguments.
     """

    if isinstance(training_script_args, dict):
        training_script_args = dict_to_args(training_script_args)
    assert isinstance(training_script_args, str)
    ...
    for local_rank in range(0, nproc_per_node):
        ...
        # 
        # Clarification:
        # sys.executable: the interpreter absolute path.
        ...
        cmd = [sys.executable,
               ...
               "--local_rank={}".format(local_rank)] + training_script_args
