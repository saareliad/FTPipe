...
if __name__ == "__main__":
    ...
    optimzer0 = torch.optim.SGD(m0.parameters(),
                                lr=1, momentum=1, dampening=0.99, weight_decay=1e-2, nesterov=True)
    ...
    config = {
        ...
        1: {  # replicated input p2mp
            'inputs': ['output0'],
            'outputs': ['output1'],
            'ranks': [torch.device('cuda:1'), torch.device('cuda:2')],
            'replicas': [m1, m2],
    ...
    print(model._shards[1][0].l.weight)
    model.backward(grads)
    time.sleep(5)
    print("master after backward")
    print(model._shards[1][0].l.weight)

    w0 = model._shards[1][0].l.weight
    w1 = model._shards[1][1].l.weight
