...
def get_generated_last_stage_scopes(model, output_file, GET_PARTITIONS_ON_CPU=True):
    ...
    layer_dict = layerDict(
        model, depth=config['depth'], basic_blocks=config['basic_blocks'])
...
class AsyncPipePartitioner:
    ...
    def partition(self, scopes_to_begin_with=set(), allowed_mistakes=2):
        """Force no recomputation for given layers in last partition until a certain allowed_mistakes is achieved. """
        self.last_partition_scopes = scopes_to_begin_with
        ...
        while current_mistakes > allowed_mistakes:
            ...
            f = force_no_recomp_fn_factory(self.last_partition_scopes)

            # Partition
            self.partition_method(force_no_recomp_scopes=f)
            self.n_runs += 1
            ...
            self.last_partition_scopes = generated_last_stage_scopes

            # stats:
            d = dict(correct=correct, fp=fp,
                     fn=fn, mistakes=current_mistakes)
            stats.append(d)
