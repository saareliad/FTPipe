...
class PipelineConfig():
    ...
    def add_stage(self,
                  ...
                  LR_scheduler_args: Dict = dict()) -> "StageConfig":
    ...
    def split(self, stage_idxs: Iterable[int]
    ...
    def realize(
            self, layers: Dict[str, Tensor], tensors: Dict[str, Tensor],
            batch_size: int
    ) -> Dict[int,
              Tuple[nn.Module, torch.
                    device, Optional[Optimizer], Optional[_LRScheduler], int]]:
        assert self.isValid()
        i = 0
        ...
        for stage_id in range(self.n_stages):
            for idx, (model, device, optimizer, lr_sched,
                ...
                rank = i + idx
                rank_to_model_args[rank] = (model, device, optimizer, lr_sched,
                                            split_size)
            i += idx + 1
...
class StageConfig():
    ...
    def _create_lr_scheduler(self, optimizer: Optional[Optimizer]
    ...
    def set_optimizer(self,
                      ...
                      optimizer_args: Dict = dict()) -> "StageConfig":
...
def createConfig(config, sample_inputs, batch_dim) -> PipelineConfig:
    config = copy(config)
    shapes, dtypes = find_shapes_and_dtypes(sample_inputs, config)
...
def find_shapes_and_dtypes(model_inputs, partition_config) -> Dict[str, torch.Size]:
    ...
    for i in range(n_partitions):
        partition_config[i]['model'].cpu()
        partition_config[i]['model'].device = 'cpu'
    ...
    parts = collections.deque(range(n_partitions))
    ...
    while len(parts) > 0:
        ...
        if all(tensor in activations
            ...
            outs = partition_config[idx]['model'].to(device)(*inputs)
            ...
            partition_config[idx]['model'].cpu()
        else:
            parts.append(idx)
