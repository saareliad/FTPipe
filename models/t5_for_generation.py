...
def decorate_args_and_kwargs_to_deivce(func, self : torch.nn.Module, device):
    """Decorate torch.nn.Module forward function by moving all inputs and outputs to device
        ...
        # NOTE: can save original function in self to remove the decoration later
     """
    ...
    return wrapper  # not sure if needed types.MethodType(wrapper, self)
...
class T5ForConditionalGeneration(T5PreTrainedModel):
    ...
    def __init__(self, config):
        ...
        # NOTE: ths is not always used but its bad habit defining functions in code so its here.
        # Also here to support spreading across devices.
