...
def create_pipeline_configuration(DEBUG=False):
    depth = -1
    basic_blocks = (Embedding,LayerNorm,Dropout,Conv1D)
    ...
    layer_scopes = ['GPT2Model/Block[6]/Attention[attn]/Conv1D[c_proj]',
    ...
    buffer_scopes = ['GPT2Model/Block[7]/Attention[attn]/Tensor[bias]',
        'GPT2Model/Block[8]/Attention[attn]/Tensor[bias]',
        'GPT2Model/Block[9]/Attention[attn]/Tensor[bias]']
    parameter_scopes = []
    

    layer_scopes = ['GPT2Model/Block[10]/LayerNorm[ln_1]',
    ...
    buffer_scopes = ['GPT2Model/Block[10]/Attention[attn]/Tensor[bias]',
    ...
    stages = {'0': {"inputs":['input0'],
        "outputs":['GPT2Model/Block[2]/MLP[mlp]/Conv1D[c_fc]', 'GPT2Model/Block[2]/aten::add5567'],
        "input_shapes":[[1, 1024]],
        "output_shapes":[[1, 1024, 3072], [1, 1024, 768]]},
            '1': {"inputs":['GPT2Model/Block[2]/MLP[mlp]/Conv1D[c_fc]', 'GPT2Model/Block[2]/aten::add5567'],
        "outputs":['GPT2Model/Block[5]/aten::add6513', 'GPT2Model/Block[6]/Attention[attn]/aten::matmul6673'],
        "input_shapes":[[1, 1024, 3072], [1, 1024, 768]],
        "output_shapes":[[1, 1024, 768], [1, 12, 1024, 64]]},
            '2': {"inputs":['GPT2Model/Block[5]/aten::add6513', 'GPT2Model/Block[6]/Attention[attn]/aten::matmul6673'],
        "outputs":['GPT2Model/Block[9]/aten::add7673'],
        "input_shapes":[[1, 1024, 768], [1, 12, 1024, 64]],
        "output_shapes":[[1, 1024, 768]]},
            '3': {"inputs":['GPT2Model/Block[9]/aten::add7673'],
    ...
    stages['0']['optimizer'] = {'type':'','args':{}}
    stages['0']['lr_scheduler'] = {'type':'','args':{}}
    ...
    stages['1']['optimizer'] = {'type':'','args':{}}
    stages['1']['lr_scheduler'] = {'type':'','args':{}}
    ...
    stages['2']['optimizer'] = {'type':'','args':{}}
    stages['2']['lr_scheduler'] = {'type':'','args':{}}
    ...
    stages['3']['optimizer'] = {'type':'','args':{}}
    stages['3']['lr_scheduler'] = {'type':'','args':{}}
...
class ModelParallel(nn.Module):
    ...
    def forward(self,input0):
        ...
        t_2, t_3 = self.stage1(t_0.to(self.stage1.device), t_1.to(self.stage1.device))
        t_4 = self.stage2(t_2.to(self.stage2.device), t_3.to(self.stage2.device))[0]
        t_5 = self.stage3(t_4.to(self.stage3.device))[0]
        return t_5
...
class Partition0(nn.Module):
    ...
    def forward(self, x0):
        ...
        return (self.l_27(self.l_26(t_32)), t_32)
...
class Partition1(nn.Module):
    def __init__(self, layers, tensors):
        ...
        # GPT2Model/Block[3]/LayerNorm[ln_1]
        self.l_2 = layers['GPT2Model/Block[3]/LayerNorm[ln_1]']
        assert isinstance(self.l_2,LayerNorm) ,f'layers[GPT2Model/Block[3]/LayerNorm[ln_1]] is expected to be of type LayerNorm but was of type {type(self.l_2)}'
        # GPT2Model/Block[3]/Attention[attn]/Conv1D[c_attn]
        self.l_3 = layers['GPT2Model/Block[3]/Attention[attn]/Conv1D[c_attn]']
        assert isinstance(self.l_3,Conv1D) ,f'layers[GPT2Model/Block[3]/Attention[attn]/Conv1D[c_attn]] is expected to be of type Conv1D but was of type {type(self.l_3)}'
        # GPT2Model/Block[3]/Attention[attn]/Dropout[attn_dropout]
        self.l_4 = layers['GPT2Model/Block[3]/Attention[attn]/Dropout[attn_dropout]']
        assert isinstance(self.l_4,Dropout) ,f'layers[GPT2Model/Block[3]/Attention[attn]/Dropout[attn_dropout]] is expected to be of type Dropout but was of type {type(self.l_4)}'
        # GPT2Model/Block[3]/Attention[attn]/Conv1D[c_proj]
        self.l_5 = layers['GPT2Model/Block[3]/Attention[attn]/Conv1D[c_proj]']
        assert isinstance(self.l_5,Conv1D) ,f'layers[GPT2Model/Block[3]/Attention[attn]/Conv1D[c_proj]] is expected to be of type Conv1D but was of type {type(self.l_5)}'
        # GPT2Model/Block[3]/Attention[attn]/Dropout[resid_dropout]
        self.l_6 = layers['GPT2Model/Block[3]/Attention[attn]/Dropout[resid_dropout]']
        assert isinstance(self.l_6,Dropout) ,f'layers[GPT2Model/Block[3]/Attention[attn]/Dropout[resid_dropout]] is expected to be of type Dropout but was of type {type(self.l_6)}'
        # GPT2Model/Block[3]/LayerNorm[ln_2]
        self.l_7 = layers['GPT2Model/Block[3]/LayerNorm[ln_2]']
        assert isinstance(self.l_7,LayerNorm) ,f'layers[GPT2Model/Block[3]/LayerNorm[ln_2]] is expected to be of type LayerNorm but was of type {type(self.l_7)}'
        # GPT2Model/Block[3]/MLP[mlp]/Conv1D[c_fc]
        self.l_8 = layers['GPT2Model/Block[3]/MLP[mlp]/Conv1D[c_fc]']
        assert isinstance(self.l_8,Conv1D) ,f'layers[GPT2Model/Block[3]/MLP[mlp]/Conv1D[c_fc]] is expected to be of type Conv1D but was of type {type(self.l_8)}'
        # GPT2Model/Block[3]/MLP[mlp]/Conv1D[c_proj]
        self.l_9 = layers['GPT2Model/Block[3]/MLP[mlp]/Conv1D[c_proj]']
        assert isinstance(self.l_9,Conv1D) ,f'layers[GPT2Model/Block[3]/MLP[mlp]/Conv1D[c_proj]] is expected to be of type Conv1D but was of type {type(self.l_9)}'
        # GPT2Model/Block[3]/MLP[mlp]/Dropout[dropout]
        self.l_10 = layers['GPT2Model/Block[3]/MLP[mlp]/Dropout[dropout]']
        assert isinstance(self.l_10,Dropout) ,f'layers[GPT2Model/Block[3]/MLP[mlp]/Dropout[dropout]] is expected to be of type Dropout but was of type {type(self.l_10)}'
        # GPT2Model/Block[4]/LayerNorm[ln_1]
        self.l_11 = layers['GPT2Model/Block[4]/LayerNorm[ln_1]']
        assert isinstance(self.l_11,LayerNorm) ,f'layers[GPT2Model/Block[4]/LayerNorm[ln_1]] is expected to be of type LayerNorm but was of type {type(self.l_11)}'
        # GPT2Model/Block[4]/Attention[attn]/Conv1D[c_attn]
        self.l_12 = layers['GPT2Model/Block[4]/Attention[attn]/Conv1D[c_attn]']
        assert isinstance(self.l_12,Conv1D) ,f'layers[GPT2Model/Block[4]/Attention[attn]/Conv1D[c_attn]] is expected to be of type Conv1D but was of type {type(self.l_12)}'
        # GPT2Model/Block[4]/Attention[attn]/Dropout[attn_dropout]
        self.l_13 = layers['GPT2Model/Block[4]/Attention[attn]/Dropout[attn_dropout]']
        assert isinstance(self.l_13,Dropout) ,f'layers[GPT2Model/Block[4]/Attention[attn]/Dropout[attn_dropout]] is expected to be of type Dropout but was of type {type(self.l_13)}'
        # GPT2Model/Block[4]/Attention[attn]/Conv1D[c_proj]
        self.l_14 = layers['GPT2Model/Block[4]/Attention[attn]/Conv1D[c_proj]']
        assert isinstance(self.l_14,Conv1D) ,f'layers[GPT2Model/Block[4]/Attention[attn]/Conv1D[c_proj]] is expected to be of type Conv1D but was of type {type(self.l_14)}'
        # GPT2Model/Block[4]/Attention[attn]/Dropout[resid_dropout]
        self.l_15 = layers['GPT2Model/Block[4]/Attention[attn]/Dropout[resid_dropout]']
        assert isinstance(self.l_15,Dropout) ,f'layers[GPT2Model/Block[4]/Attention[attn]/Dropout[resid_dropout]] is expected to be of type Dropout but was of type {type(self.l_15)}'
        # GPT2Model/Block[4]/LayerNorm[ln_2]
        self.l_16 = layers['GPT2Model/Block[4]/LayerNorm[ln_2]']
        assert isinstance(self.l_16,LayerNorm) ,f'layers[GPT2Model/Block[4]/LayerNorm[ln_2]] is expected to be of type LayerNorm but was of type {type(self.l_16)}'
        # GPT2Model/Block[4]/MLP[mlp]/Conv1D[c_fc]
        self.l_17 = layers['GPT2Model/Block[4]/MLP[mlp]/Conv1D[c_fc]']
        assert isinstance(self.l_17,Conv1D) ,f'layers[GPT2Model/Block[4]/MLP[mlp]/Conv1D[c_fc]] is expected to be of type Conv1D but was of type {type(self.l_17)}'
        # GPT2Model/Block[4]/MLP[mlp]/Conv1D[c_proj]
        self.l_18 = layers['GPT2Model/Block[4]/MLP[mlp]/Conv1D[c_proj]']
        assert isinstance(self.l_18,Conv1D) ,f'layers[GPT2Model/Block[4]/MLP[mlp]/Conv1D[c_proj]] is expected to be of type Conv1D but was of type {type(self.l_18)}'
        # GPT2Model/Block[4]/MLP[mlp]/Dropout[dropout]
        self.l_19 = layers['GPT2Model/Block[4]/MLP[mlp]/Dropout[dropout]']
        assert isinstance(self.l_19,Dropout) ,f'layers[GPT2Model/Block[4]/MLP[mlp]/Dropout[dropout]] is expected to be of type Dropout but was of type {type(self.l_19)}'
        # GPT2Model/Block[5]/LayerNorm[ln_1]
        self.l_20 = layers['GPT2Model/Block[5]/LayerNorm[ln_1]']
        assert isinstance(self.l_20,LayerNorm) ,f'layers[GPT2Model/Block[5]/LayerNorm[ln_1]] is expected to be of type LayerNorm but was of type {type(self.l_20)}'
        # GPT2Model/Block[5]/Attention[attn]/Conv1D[c_attn]
        self.l_21 = layers['GPT2Model/Block[5]/Attention[attn]/Conv1D[c_attn]']
        assert isinstance(self.l_21,Conv1D) ,f'layers[GPT2Model/Block[5]/Attention[attn]/Conv1D[c_attn]] is expected to be of type Conv1D but was of type {type(self.l_21)}'
        # GPT2Model/Block[5]/Attention[attn]/Dropout[attn_dropout]
        self.l_22 = layers['GPT2Model/Block[5]/Attention[attn]/Dropout[attn_dropout]']
        assert isinstance(self.l_22,Dropout) ,f'layers[GPT2Model/Block[5]/Attention[attn]/Dropout[attn_dropout]] is expected to be of type Dropout but was of type {type(self.l_22)}'
        # GPT2Model/Block[5]/Attention[attn]/Conv1D[c_proj]
        self.l_23 = layers['GPT2Model/Block[5]/Attention[attn]/Conv1D[c_proj]']
        assert isinstance(self.l_23,Conv1D) ,f'layers[GPT2Model/Block[5]/Attention[attn]/Conv1D[c_proj]] is expected to be of type Conv1D but was of type {type(self.l_23)}'
        # GPT2Model/Block[5]/Attention[attn]/Dropout[resid_dropout]
        self.l_24 = layers['GPT2Model/Block[5]/Attention[attn]/Dropout[resid_dropout]']
        assert isinstance(self.l_24,Dropout) ,f'layers[GPT2Model/Block[5]/Attention[attn]/Dropout[resid_dropout]] is expected to be of type Dropout but was of type {type(self.l_24)}'
        # GPT2Model/Block[5]/LayerNorm[ln_2]
        self.l_25 = layers['GPT2Model/Block[5]/LayerNorm[ln_2]']
        assert isinstance(self.l_25,LayerNorm) ,f'layers[GPT2Model/Block[5]/LayerNorm[ln_2]] is expected to be of type LayerNorm but was of type {type(self.l_25)}'
        # GPT2Model/Block[5]/MLP[mlp]/Conv1D[c_fc]
        self.l_26 = layers['GPT2Model/Block[5]/MLP[mlp]/Conv1D[c_fc]']
        assert isinstance(self.l_26,Conv1D) ,f'layers[GPT2Model/Block[5]/MLP[mlp]/Conv1D[c_fc]] is expected to be of type Conv1D but was of type {type(self.l_26)}'
        # GPT2Model/Block[5]/MLP[mlp]/Conv1D[c_proj]
        self.l_27 = layers['GPT2Model/Block[5]/MLP[mlp]/Conv1D[c_proj]']
        assert isinstance(self.l_27,Conv1D) ,f'layers[GPT2Model/Block[5]/MLP[mlp]/Conv1D[c_proj]] is expected to be of type Conv1D but was of type {type(self.l_27)}'
        # GPT2Model/Block[5]/MLP[mlp]/Dropout[dropout]
        self.l_28 = layers['GPT2Model/Block[5]/MLP[mlp]/Dropout[dropout]']
        assert isinstance(self.l_28,Dropout) ,f'layers[GPT2Model/Block[5]/MLP[mlp]/Dropout[dropout]] is expected to be of type Dropout but was of type {type(self.l_28)}'
        # GPT2Model/Block[6]/LayerNorm[ln_1]
        self.l_29 = layers['GPT2Model/Block[6]/LayerNorm[ln_1]']
        assert isinstance(self.l_29,LayerNorm) ,f'layers[GPT2Model/Block[6]/LayerNorm[ln_1]] is expected to be of type LayerNorm but was of type {type(self.l_29)}'
        # GPT2Model/Block[6]/Attention[attn]/Conv1D[c_attn]
        self.l_30 = layers['GPT2Model/Block[6]/Attention[attn]/Conv1D[c_attn]']
        assert isinstance(self.l_30,Conv1D) ,f'layers[GPT2Model/Block[6]/Attention[attn]/Conv1D[c_attn]] is expected to be of type Conv1D but was of type {type(self.l_30)}'
        # GPT2Model/Block[6]/Attention[attn]/Dropout[attn_dropout]
        self.l_31 = layers['GPT2Model/Block[6]/Attention[attn]/Dropout[attn_dropout]']
        assert isinstance(self.l_31,Dropout) ,f'layers[GPT2Model/Block[6]/Attention[attn]/Dropout[attn_dropout]] is expected to be of type Dropout but was of type {type(self.l_31)}'
        ...
        # GPT2Model/Block[6]/Attention[attn]/Tensor[bias]
        self.register_buffer('b_3',tensors['GPT2Model/Block[6]/Attention[attn]/Tensor[bias]'])
        ...
        self.lookup = { 'l_0': '2.mlp.c_proj',
                        ...
                        'l_2': '3.ln_1',
                        'l_3': '3.attn.c_attn',
                        'l_4': '3.attn.attn_dropout',
                        'l_5': '3.attn.c_proj',
                        'l_6': '3.attn.resid_dropout',
                        'l_7': '3.ln_2',
                        'l_8': '3.mlp.c_fc',
                        'l_9': '3.mlp.c_proj',
                        'l_10': '3.mlp.dropout',
                        'l_11': '4.ln_1',
                        'l_12': '4.attn.c_attn',
                        'l_13': '4.attn.attn_dropout',
                        'l_14': '4.attn.c_proj',
                        'l_15': '4.attn.resid_dropout',
                        'l_16': '4.ln_2',
                        'l_17': '4.mlp.c_fc',
                        'l_18': '4.mlp.c_proj',
                        'l_19': '4.mlp.dropout',
                        'l_20': '5.ln_1',
                        'l_21': '5.attn.c_attn',
                        'l_22': '5.attn.attn_dropout',
                        'l_23': '5.attn.c_proj',
                        'l_24': '5.attn.resid_dropout',
                        'l_25': '5.ln_2',
                        'l_26': '5.mlp.c_fc',
                        'l_27': '5.mlp.c_proj',
                        'l_28': '5.mlp.dropout',
    ...
    def forward(self, x0, x1):
        ...
        # GPT2Model/Block[3]/LayerNorm[ln_1] <=> self.l_2
        # GPT2Model/Block[3]/Attention[attn]/Conv1D[c_attn] <=> self.l_3
        # GPT2Model/Block[3]/Attention[attn]/Dropout[attn_dropout] <=> self.l_4
        # GPT2Model/Block[3]/Attention[attn]/Conv1D[c_proj] <=> self.l_5
        # GPT2Model/Block[3]/Attention[attn]/Dropout[resid_dropout] <=> self.l_6
        # GPT2Model/Block[3]/LayerNorm[ln_2] <=> self.l_7
        # GPT2Model/Block[3]/MLP[mlp]/Conv1D[c_fc] <=> self.l_8
        # GPT2Model/Block[3]/MLP[mlp]/Conv1D[c_proj] <=> self.l_9
        # GPT2Model/Block[3]/MLP[mlp]/Dropout[dropout] <=> self.l_10
        # GPT2Model/Block[4]/LayerNorm[ln_1] <=> self.l_11
        # GPT2Model/Block[4]/Attention[attn]/Conv1D[c_attn] <=> self.l_12
        # GPT2Model/Block[4]/Attention[attn]/Dropout[attn_dropout] <=> self.l_13
        # GPT2Model/Block[4]/Attention[attn]/Conv1D[c_proj] <=> self.l_14
        # GPT2Model/Block[4]/Attention[attn]/Dropout[resid_dropout] <=> self.l_15
        # GPT2Model/Block[4]/LayerNorm[ln_2] <=> self.l_16
        # GPT2Model/Block[4]/MLP[mlp]/Conv1D[c_fc] <=> self.l_17
        # GPT2Model/Block[4]/MLP[mlp]/Conv1D[c_proj] <=> self.l_18
        # GPT2Model/Block[4]/MLP[mlp]/Dropout[dropout] <=> self.l_19
        # GPT2Model/Block[5]/LayerNorm[ln_1] <=> self.l_20
        # GPT2Model/Block[5]/Attention[attn]/Conv1D[c_attn] <=> self.l_21
        # GPT2Model/Block[5]/Attention[attn]/Dropout[attn_dropout] <=> self.l_22
        # GPT2Model/Block[5]/Attention[attn]/Conv1D[c_proj] <=> self.l_23
        # GPT2Model/Block[5]/Attention[attn]/Dropout[resid_dropout] <=> self.l_24
        # GPT2Model/Block[5]/LayerNorm[ln_2] <=> self.l_25
        # GPT2Model/Block[5]/MLP[mlp]/Conv1D[c_fc] <=> self.l_26
        # GPT2Model/Block[5]/MLP[mlp]/Conv1D[c_proj] <=> self.l_27
        # GPT2Model/Block[5]/MLP[mlp]/Dropout[dropout] <=> self.l_28
        # GPT2Model/Block[6]/LayerNorm[ln_1] <=> self.l_29
        # GPT2Model/Block[6]/Attention[attn]/Conv1D[c_attn] <=> self.l_30
        # GPT2Model/Block[6]/Attention[attn]/Dropout[attn_dropout] <=> self.l_31
        ...
        # GPT2Model/Block[6]/Attention[attn]/Tensor[bias] <=> self.b_3
        # GPT2Model/Block[2]/MLP[mlp]/Conv1D[c_fc] <=> x0
        ...
        t_34 = Tensor.split(self.l_30(self.l_29(t_33)), split_size=768, dim=2)
        t_35 = t_34[0]
        t_36 = t_34[1]
        t_37 = t_34[2]
        # calling torch.div with arguments:
        ...
        t_38 = torch.div(input=Tensor.matmul(Tensor.permute(Tensor.view(t_35, size=[Tensor.size(t_35, dim=0), Tensor.size(t_35, dim=1), 12, torch.div(input=Tensor.size(t_35, dim=-1), other=12)]), dims=[0, 2, 1, 3]), other=Tensor.permute(Tensor.view(t_36, size=[Tensor.size(t_36, dim=0), Tensor.size(t_36, dim=1), 12, torch.div(input=Tensor.size(t_36, dim=-1), other=12)]), dims=[0, 2, 3, 1])), other=8.0)
        # calling Tensor.size with arguments:
        ...
        # GPT2Model/Block[6]/Attention[attn]/aten::matmul6673
        return (t_33, Tensor.matmul(self.l_31(Tensor.softmax(torch.sub(input=torch.mul(input=t_38, other=t_40), other=torch.mul(input=torch.rsub(t_40, other=1, alpha=1), other=10000.0)), dim=-1, dtype=None)), other=Tensor.permute(Tensor.view(t_37, size=[Tensor.size(t_37, dim=0), Tensor.size(t_37, dim=1), 12, torch.div(input=Tensor.size(t_37, dim=-1), other=12)]), dims=[0, 2, 1, 3])))
...
class Partition2(nn.Module):
    def __init__(self, layers, tensors):
        ...
        self.l_0 = layers['GPT2Model/Block[6]/Attention[attn]/Conv1D[c_proj]']
        assert isinstance(self.l_0,Conv1D) ,f'layers[GPT2Model/Block[6]/Attention[attn]/Conv1D[c_proj]] is expected to be of type Conv1D but was of type {type(self.l_0)}'
        # GPT2Model/Block[6]/Attention[attn]/Dropout[resid_dropout]
        self.l_1 = layers['GPT2Model/Block[6]/Attention[attn]/Dropout[resid_dropout]']
        assert isinstance(self.l_1,Dropout) ,f'layers[GPT2Model/Block[6]/Attention[attn]/Dropout[resid_dropout]] is expected to be of type Dropout but was of type {type(self.l_1)}'
        # GPT2Model/Block[6]/LayerNorm[ln_2]
        self.l_2 = layers['GPT2Model/Block[6]/LayerNorm[ln_2]']
        assert isinstance(self.l_2,LayerNorm) ,f'layers[GPT2Model/Block[6]/LayerNorm[ln_2]] is expected to be of type LayerNorm but was of type {type(self.l_2)}'
        # GPT2Model/Block[6]/MLP[mlp]/Conv1D[c_fc]
        self.l_3 = layers['GPT2Model/Block[6]/MLP[mlp]/Conv1D[c_fc]']
        assert isinstance(self.l_3,Conv1D) ,f'layers[GPT2Model/Block[6]/MLP[mlp]/Conv1D[c_fc]] is expected to be of type Conv1D but was of type {type(self.l_3)}'
        # GPT2Model/Block[6]/MLP[mlp]/Conv1D[c_proj]
        self.l_4 = layers['GPT2Model/Block[6]/MLP[mlp]/Conv1D[c_proj]']
        assert isinstance(self.l_4,Conv1D) ,f'layers[GPT2Model/Block[6]/MLP[mlp]/Conv1D[c_proj]] is expected to be of type Conv1D but was of type {type(self.l_4)}'
        # GPT2Model/Block[6]/MLP[mlp]/Dropout[dropout]
        self.l_5 = layers['GPT2Model/Block[6]/MLP[mlp]/Dropout[dropout]']
        assert isinstance(self.l_5,Dropout) ,f'layers[GPT2Model/Block[6]/MLP[mlp]/Dropout[dropout]] is expected to be of type Dropout but was of type {type(self.l_5)}'
        # GPT2Model/Block[7]/LayerNorm[ln_1]
        self.l_6 = layers['GPT2Model/Block[7]/LayerNorm[ln_1]']
        assert isinstance(self.l_6,LayerNorm) ,f'layers[GPT2Model/Block[7]/LayerNorm[ln_1]] is expected to be of type LayerNorm but was of type {type(self.l_6)}'
        # GPT2Model/Block[7]/Attention[attn]/Conv1D[c_attn]
        self.l_7 = layers['GPT2Model/Block[7]/Attention[attn]/Conv1D[c_attn]']
        assert isinstance(self.l_7,Conv1D) ,f'layers[GPT2Model/Block[7]/Attention[attn]/Conv1D[c_attn]] is expected to be of type Conv1D but was of type {type(self.l_7)}'
        # GPT2Model/Block[7]/Attention[attn]/Dropout[attn_dropout]
        self.l_8 = layers['GPT2Model/Block[7]/Attention[attn]/Dropout[attn_dropout]']
        assert isinstance(self.l_8,Dropout) ,f'layers[GPT2Model/Block[7]/Attention[attn]/Dropout[attn_dropout]] is expected to be of type Dropout but was of type {type(self.l_8)}'
        # GPT2Model/Block[7]/Attention[attn]/Conv1D[c_proj]
        self.l_9 = layers['GPT2Model/Block[7]/Attention[attn]/Conv1D[c_proj]']
        assert isinstance(self.l_9,Conv1D) ,f'layers[GPT2Model/Block[7]/Attention[attn]/Conv1D[c_proj]] is expected to be of type Conv1D but was of type {type(self.l_9)}'
        # GPT2Model/Block[7]/Attention[attn]/Dropout[resid_dropout]
        self.l_10 = layers['GPT2Model/Block[7]/Attention[attn]/Dropout[resid_dropout]']
        assert isinstance(self.l_10,Dropout) ,f'layers[GPT2Model/Block[7]/Attention[attn]/Dropout[resid_dropout]] is expected to be of type Dropout but was of type {type(self.l_10)}'
        # GPT2Model/Block[7]/LayerNorm[ln_2]
        self.l_11 = layers['GPT2Model/Block[7]/LayerNorm[ln_2]']
        assert isinstance(self.l_11,LayerNorm) ,f'layers[GPT2Model/Block[7]/LayerNorm[ln_2]] is expected to be of type LayerNorm but was of type {type(self.l_11)}'
        # GPT2Model/Block[7]/MLP[mlp]/Conv1D[c_fc]
        self.l_12 = layers['GPT2Model/Block[7]/MLP[mlp]/Conv1D[c_fc]']
        assert isinstance(self.l_12,Conv1D) ,f'layers[GPT2Model/Block[7]/MLP[mlp]/Conv1D[c_fc]] is expected to be of type Conv1D but was of type {type(self.l_12)}'
        # GPT2Model/Block[7]/MLP[mlp]/Conv1D[c_proj]
        self.l_13 = layers['GPT2Model/Block[7]/MLP[mlp]/Conv1D[c_proj]']
        assert isinstance(self.l_13,Conv1D) ,f'layers[GPT2Model/Block[7]/MLP[mlp]/Conv1D[c_proj]] is expected to be of type Conv1D but was of type {type(self.l_13)}'
        # GPT2Model/Block[7]/MLP[mlp]/Dropout[dropout]
        self.l_14 = layers['GPT2Model/Block[7]/MLP[mlp]/Dropout[dropout]']
        assert isinstance(self.l_14,Dropout) ,f'layers[GPT2Model/Block[7]/MLP[mlp]/Dropout[dropout]] is expected to be of type Dropout but was of type {type(self.l_14)}'
        # GPT2Model/Block[8]/LayerNorm[ln_1]
        self.l_15 = layers['GPT2Model/Block[8]/LayerNorm[ln_1]']
        assert isinstance(self.l_15,LayerNorm) ,f'layers[GPT2Model/Block[8]/LayerNorm[ln_1]] is expected to be of type LayerNorm but was of type {type(self.l_15)}'
        # GPT2Model/Block[8]/Attention[attn]/Conv1D[c_attn]
        self.l_16 = layers['GPT2Model/Block[8]/Attention[attn]/Conv1D[c_attn]']
        assert isinstance(self.l_16,Conv1D) ,f'layers[GPT2Model/Block[8]/Attention[attn]/Conv1D[c_attn]] is expected to be of type Conv1D but was of type {type(self.l_16)}'
        # GPT2Model/Block[8]/Attention[attn]/Dropout[attn_dropout]
        self.l_17 = layers['GPT2Model/Block[8]/Attention[attn]/Dropout[attn_dropout]']
        assert isinstance(self.l_17,Dropout) ,f'layers[GPT2Model/Block[8]/Attention[attn]/Dropout[attn_dropout]] is expected to be of type Dropout but was of type {type(self.l_17)}'
        # GPT2Model/Block[8]/Attention[attn]/Conv1D[c_proj]
        self.l_18 = layers['GPT2Model/Block[8]/Attention[attn]/Conv1D[c_proj]']
        assert isinstance(self.l_18,Conv1D) ,f'layers[GPT2Model/Block[8]/Attention[attn]/Conv1D[c_proj]] is expected to be of type Conv1D but was of type {type(self.l_18)}'
        # GPT2Model/Block[8]/Attention[attn]/Dropout[resid_dropout]
        self.l_19 = layers['GPT2Model/Block[8]/Attention[attn]/Dropout[resid_dropout]']
        assert isinstance(self.l_19,Dropout) ,f'layers[GPT2Model/Block[8]/Attention[attn]/Dropout[resid_dropout]] is expected to be of type Dropout but was of type {type(self.l_19)}'
        # GPT2Model/Block[8]/LayerNorm[ln_2]
        self.l_20 = layers['GPT2Model/Block[8]/LayerNorm[ln_2]']
        assert isinstance(self.l_20,LayerNorm) ,f'layers[GPT2Model/Block[8]/LayerNorm[ln_2]] is expected to be of type LayerNorm but was of type {type(self.l_20)}'
        # GPT2Model/Block[8]/MLP[mlp]/Conv1D[c_fc]
        self.l_21 = layers['GPT2Model/Block[8]/MLP[mlp]/Conv1D[c_fc]']
        assert isinstance(self.l_21,Conv1D) ,f'layers[GPT2Model/Block[8]/MLP[mlp]/Conv1D[c_fc]] is expected to be of type Conv1D but was of type {type(self.l_21)}'
        # GPT2Model/Block[8]/MLP[mlp]/Conv1D[c_proj]
        self.l_22 = layers['GPT2Model/Block[8]/MLP[mlp]/Conv1D[c_proj]']
        assert isinstance(self.l_22,Conv1D) ,f'layers[GPT2Model/Block[8]/MLP[mlp]/Conv1D[c_proj]] is expected to be of type Conv1D but was of type {type(self.l_22)}'
        # GPT2Model/Block[8]/MLP[mlp]/Dropout[dropout]
        self.l_23 = layers['GPT2Model/Block[8]/MLP[mlp]/Dropout[dropout]']
        assert isinstance(self.l_23,Dropout) ,f'layers[GPT2Model/Block[8]/MLP[mlp]/Dropout[dropout]] is expected to be of type Dropout but was of type {type(self.l_23)}'
        # GPT2Model/Block[9]/LayerNorm[ln_1]
        self.l_24 = layers['GPT2Model/Block[9]/LayerNorm[ln_1]']
        assert isinstance(self.l_24,LayerNorm) ,f'layers[GPT2Model/Block[9]/LayerNorm[ln_1]] is expected to be of type LayerNorm but was of type {type(self.l_24)}'
        # GPT2Model/Block[9]/Attention[attn]/Conv1D[c_attn]
        self.l_25 = layers['GPT2Model/Block[9]/Attention[attn]/Conv1D[c_attn]']
        assert isinstance(self.l_25,Conv1D) ,f'layers[GPT2Model/Block[9]/Attention[attn]/Conv1D[c_attn]] is expected to be of type Conv1D but was of type {type(self.l_25)}'
        # GPT2Model/Block[9]/Attention[attn]/Dropout[attn_dropout]
        self.l_26 = layers['GPT2Model/Block[9]/Attention[attn]/Dropout[attn_dropout]']
        assert isinstance(self.l_26,Dropout) ,f'layers[GPT2Model/Block[9]/Attention[attn]/Dropout[attn_dropout]] is expected to be of type Dropout but was of type {type(self.l_26)}'
        # GPT2Model/Block[9]/Attention[attn]/Conv1D[c_proj]
        self.l_27 = layers['GPT2Model/Block[9]/Attention[attn]/Conv1D[c_proj]']
        assert isinstance(self.l_27,Conv1D) ,f'layers[GPT2Model/Block[9]/Attention[attn]/Conv1D[c_proj]] is expected to be of type Conv1D but was of type {type(self.l_27)}'
        # GPT2Model/Block[9]/Attention[attn]/Dropout[resid_dropout]
        self.l_28 = layers['GPT2Model/Block[9]/Attention[attn]/Dropout[resid_dropout]']
        assert isinstance(self.l_28,Dropout) ,f'layers[GPT2Model/Block[9]/Attention[attn]/Dropout[resid_dropout]] is expected to be of type Dropout but was of type {type(self.l_28)}'
        # GPT2Model/Block[9]/LayerNorm[ln_2]
        self.l_29 = layers['GPT2Model/Block[9]/LayerNorm[ln_2]']
        assert isinstance(self.l_29,LayerNorm) ,f'layers[GPT2Model/Block[9]/LayerNorm[ln_2]] is expected to be of type LayerNorm but was of type {type(self.l_29)}'
        # GPT2Model/Block[9]/MLP[mlp]/Conv1D[c_fc]
        self.l_30 = layers['GPT2Model/Block[9]/MLP[mlp]/Conv1D[c_fc]']
        assert isinstance(self.l_30,Conv1D) ,f'layers[GPT2Model/Block[9]/MLP[mlp]/Conv1D[c_fc]] is expected to be of type Conv1D but was of type {type(self.l_30)}'
        # GPT2Model/Block[9]/MLP[mlp]/Conv1D[c_proj]
        self.l_31 = layers['GPT2Model/Block[9]/MLP[mlp]/Conv1D[c_proj]']
        assert isinstance(self.l_31,Conv1D) ,f'layers[GPT2Model/Block[9]/MLP[mlp]/Conv1D[c_proj]] is expected to be of type Conv1D but was of type {type(self.l_31)}'
        # GPT2Model/Block[9]/MLP[mlp]/Dropout[dropout]
        self.l_32 = layers['GPT2Model/Block[9]/MLP[mlp]/Dropout[dropout]']
        assert isinstance(self.l_32,Dropout) ,f'layers[GPT2Model/Block[9]/MLP[mlp]/Dropout[dropout]] is expected to be of type Dropout but was of type {type(self.l_32)}'

        # initializing partition buffers
        # GPT2Model/Block[7]/Attention[attn]/Tensor[bias]
        self.register_buffer('b_0',tensors['GPT2Model/Block[7]/Attention[attn]/Tensor[bias]'])
        # GPT2Model/Block[8]/Attention[attn]/Tensor[bias]
        self.register_buffer('b_1',tensors['GPT2Model/Block[8]/Attention[attn]/Tensor[bias]'])
        # GPT2Model/Block[9]/Attention[attn]/Tensor[bias]
        self.register_buffer('b_2',tensors['GPT2Model/Block[9]/Attention[attn]/Tensor[bias]'])
        
        # initializing partition parameters
        self.device = torch.device('cuda:2')
        self.lookup = { 'l_0': '6.attn.c_proj',
                        'l_1': '6.attn.resid_dropout',
                        'l_2': '6.ln_2',
                        'l_3': '6.mlp.c_fc',
                        'l_4': '6.mlp.c_proj',
                        'l_5': '6.mlp.dropout',
                        'l_6': '7.ln_1',
                        'l_7': '7.attn.c_attn',
                        'l_8': '7.attn.attn_dropout',
                        'l_9': '7.attn.c_proj',
                        'l_10': '7.attn.resid_dropout',
                        'l_11': '7.ln_2',
                        'l_12': '7.mlp.c_fc',
                        'l_13': '7.mlp.c_proj',
                        'l_14': '7.mlp.dropout',
                        'l_15': '8.ln_1',
                        'l_16': '8.attn.c_attn',
                        'l_17': '8.attn.attn_dropout',
                        'l_18': '8.attn.c_proj',
                        'l_19': '8.attn.resid_dropout',
                        'l_20': '8.ln_2',
                        'l_21': '8.mlp.c_fc',
                        'l_22': '8.mlp.c_proj',
                        'l_23': '8.mlp.dropout',
                        'l_24': '9.ln_1',
                        'l_25': '9.attn.c_attn',
                        'l_26': '9.attn.attn_dropout',
                        'l_27': '9.attn.c_proj',
                        'l_28': '9.attn.resid_dropout',
                        'l_29': '9.ln_2',
                        'l_30': '9.mlp.c_fc',
                        'l_31': '9.mlp.c_proj',
                        'l_32': '9.mlp.dropout',
    ...
    def forward(self, x0, x1):
        # GPT2Model/Block[6]/Attention[attn]/Conv1D[c_proj] <=> self.l_0
        # GPT2Model/Block[6]/Attention[attn]/Dropout[resid_dropout] <=> self.l_1
        # GPT2Model/Block[6]/LayerNorm[ln_2] <=> self.l_2
        # GPT2Model/Block[6]/MLP[mlp]/Conv1D[c_fc] <=> self.l_3
        # GPT2Model/Block[6]/MLP[mlp]/Conv1D[c_proj] <=> self.l_4
        # GPT2Model/Block[6]/MLP[mlp]/Dropout[dropout] <=> self.l_5
        # GPT2Model/Block[7]/LayerNorm[ln_1] <=> self.l_6
        # GPT2Model/Block[7]/Attention[attn]/Conv1D[c_attn] <=> self.l_7
        # GPT2Model/Block[7]/Attention[attn]/Dropout[attn_dropout] <=> self.l_8
        # GPT2Model/Block[7]/Attention[attn]/Conv1D[c_proj] <=> self.l_9
        # GPT2Model/Block[7]/Attention[attn]/Dropout[resid_dropout] <=> self.l_10
        # GPT2Model/Block[7]/LayerNorm[ln_2] <=> self.l_11
        # GPT2Model/Block[7]/MLP[mlp]/Conv1D[c_fc] <=> self.l_12
        # GPT2Model/Block[7]/MLP[mlp]/Conv1D[c_proj] <=> self.l_13
        # GPT2Model/Block[7]/MLP[mlp]/Dropout[dropout] <=> self.l_14
        # GPT2Model/Block[8]/LayerNorm[ln_1] <=> self.l_15
        # GPT2Model/Block[8]/Attention[attn]/Conv1D[c_attn] <=> self.l_16
        # GPT2Model/Block[8]/Attention[attn]/Dropout[attn_dropout] <=> self.l_17
        # GPT2Model/Block[8]/Attention[attn]/Conv1D[c_proj] <=> self.l_18
        # GPT2Model/Block[8]/Attention[attn]/Dropout[resid_dropout] <=> self.l_19
        # GPT2Model/Block[8]/LayerNorm[ln_2] <=> self.l_20
        # GPT2Model/Block[8]/MLP[mlp]/Conv1D[c_fc] <=> self.l_21
        # GPT2Model/Block[8]/MLP[mlp]/Conv1D[c_proj] <=> self.l_22
        # GPT2Model/Block[8]/MLP[mlp]/Dropout[dropout] <=> self.l_23
        # GPT2Model/Block[9]/LayerNorm[ln_1] <=> self.l_24
        # GPT2Model/Block[9]/Attention[attn]/Conv1D[c_attn] <=> self.l_25
        # GPT2Model/Block[9]/Attention[attn]/Dropout[attn_dropout] <=> self.l_26
        # GPT2Model/Block[9]/Attention[attn]/Conv1D[c_proj] <=> self.l_27
        # GPT2Model/Block[9]/Attention[attn]/Dropout[resid_dropout] <=> self.l_28
        # GPT2Model/Block[9]/LayerNorm[ln_2] <=> self.l_29
        # GPT2Model/Block[9]/MLP[mlp]/Conv1D[c_fc] <=> self.l_30
        # GPT2Model/Block[9]/MLP[mlp]/Conv1D[c_proj] <=> self.l_31
        # GPT2Model/Block[9]/MLP[mlp]/Dropout[dropout] <=> self.l_32
        # GPT2Model/Block[7]/Attention[attn]/Tensor[bias] <=> self.b_0
        # GPT2Model/Block[8]/Attention[attn]/Tensor[bias] <=> self.b_1
        # GPT2Model/Block[9]/Attention[attn]/Tensor[bias] <=> self.b_2
        # GPT2Model/Block[5]/aten::add6513 <=> x0
        # GPT2Model/Block[6]/Attention[attn]/aten::matmul6673 <=> x1
...
class Partition3(nn.Module):
    ...
    def forward(self, x0):
        ...
        # GPT2Model/Block[9]/aten::add7673 <=> x0
