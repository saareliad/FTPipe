"""AutoGenerated with:
python partitioning_script.py t5 --model_name_or_path t5-3b --t5_task squad1 --lmhead --n_iter 10 --analysis_batch_size 8 --partitioning_batch_size 8 --precompute_masks --stateless_tied --lmhead --n_partitions 8 --max_seq_length 512 --answer_max_seq_length 4 --basic_blocks T5Block --partitioning_method 2DBIN --n_clusters 2 --save_memory_mode --disable_op_profiling
"""
import math
...
# partition 0 {'inputs': {'input_ids'}, 'outputs': {9, 1}}
# partition 1 {'inputs': {0, 'attention_mask'}, 'outputs': {2, 3, 4, 5, 6, 7, 8}}
# partition 2 {'inputs': {1, 'attention_mask'}, 'outputs': {3}}
# partition 3 {'inputs': {1, 2, 'attention_mask'}, 'outputs': {4}}
# partition 4 {'inputs': {1, 3, 'attention_mask'}, 'outputs': {5}}
# partition 5 {'inputs': {1, 'attention_mask', 4}, 'outputs': {6}}
# partition 6 {'inputs': {1, 'attention_mask', 5}, 'outputs': {7}}
# partition 7 {'inputs': {1, 'attention_mask', 6}, 'outputs': {8}}
# partition 8 {'inputs': {1, 'attention_mask', 7}, 'outputs': {9, 10, 11, 12, 13, 14, 15}}
# partition 9 {'inputs': {0, 'decoder_input_ids', 8, 'inverted_encoder_attention_mask', 'decoder_attention_mask'}, 'outputs': {10, 11, 12, 13, 14, 15}}
# partition 10 {'inputs': {8, 9, 'decoder_attention_mask', 'inverted_encoder_attention_mask'}, 'outputs': {11}}
# partition 11 {'inputs': {8, 9, 10, 'inverted_encoder_attention_mask', 'decoder_attention_mask'}, 'outputs': {12}}
# partition 12 {'inputs': {8, 9, 11, 'inverted_encoder_attention_mask', 'decoder_attention_mask'}, 'outputs': {13}}
# partition 13 {'inputs': {8, 9, 12, 'inverted_encoder_attention_mask', 'decoder_attention_mask'}, 'outputs': {14}}
# partition 14 {'inputs': {8, 9, 13, 'inverted_encoder_attention_mask', 'decoder_attention_mask'}, 'outputs': {15}}
# partition 15 {'inputs': {8, 9, 'inverted_encoder_attention_mask', 14, 'lm_labels', 'decoder_attention_mask'}, 'outputs': {'output'}}
# model outputs {15}
...
def create_pipeline_configuration(DEBUG=False, batch_size=8):
    config = {
        'batch_dim': 0,
        'depth': 10000,
        'basic_blocks': (StatelessEmbedding,Linear,CrossEntropyLoss,T5LayerNorm,T5Block,Dropout),
        'model_inputs': {
            'attention_mask': {
                'shape': torch.Size([8, 1, 1, 512]),
                'dtype': torch.float32,
                'is_batched': True,
                'used_by': [1, 2, 3, 4, 5, 6, 7, 8]},
            'decoder_attention_mask': {
                'shape': torch.Size([8, 1, 4, 4]),
                'dtype': torch.float32,
                'is_batched': True,
...
class Partition1(nn.Module):
    ...
    def forward(self, *args):
        ...
        # T5ForConditionalGeneration/T5Stack[encoder]/T5Block[1] <=> self.l_1
        # T5ForConditionalGeneration/T5Stack[encoder]/T5Block[2] <=> self.l_2
        ...
        t_1 = t_0[0]
        t_0 = t_0[1]
        t_1 = self.l_1(t_1, attention_mask=attention_mask, position_bias=t_0, encoder_hidden_states=None, encoder_attention_mask=None, encoder_decoder_position_bias=None)
        t_1 = self.l_2(t_1, attention_mask=attention_mask, position_bias=t_0, encoder_hidden_states=None, encoder_attention_mask=None, encoder_decoder_position_bias=None)
        # returning:
...
unique_stages_on_same_gpu: [{6, 15}, {0, 8}, {1, 10}, {2, 12}, {3, 13}, {4, 14}, {11, 5}, {9, 7}]
"stage_to_device_map": [0, 1, 2, 3, 4, 5, 6, 7, 0, 7, 1, 5, 2, 3, 4, 6],
...
forward {0: 163.78, 1: 206.95, 2: 204.33, 3: 204.48, 4: 204.33, 5: 204.22, 6: 189.21, 7: 189.41}
backward {0: 421.27, 1: 495.48, 2: 516.16, 3: 516.29, 4: 516.03, 5: 515.82, 6: 471.8, 7: 481.93}

Assuming bandwidth of 12 GBps between GPUs

communication volumes size of activations of each partition
0: input size:'285.26 MB', recieve_time:'23.77 ms', out:'165.04 MB', send time:'13.75 ms'
1: input size:'35.83 MB', recieve_time:'2.99 ms', out:'304.23 MB', send time:'25.35 ms'
2: input size:'304.27 MB', recieve_time:'25.36 ms', out:'304.23 MB', send time:'25.35 ms'
3: input size:'304.27 MB', recieve_time:'25.36 ms', out:'304.23 MB', send time:'25.35 ms'
4: input size:'304.27 MB', recieve_time:'25.36 ms', out:'304.23 MB', send time:'25.35 ms'
5: input size:'304.27 MB', recieve_time:'25.36 ms', out:'304.23 MB', send time:'25.35 ms'
6: input size:'304.27 MB', recieve_time:'25.36 ms', out:'285.21 MB', send time:'23.77 ms'
7: input size:'433.51 MB', recieve_time:'36.13 ms', out:'304.23 MB', send time:'25.35 ms'

Compuatation Communication ratio (comp/(comp+comm)):
forward {0: 0.92, 1: 0.88, 2: 0.88, 3: 0.88, 4: 0.88, 5: 0.88, 6: 0.87, 7: 0.87} 
backward {0: 0.94, 1: 0.99, 2: 0.95, 3: 0.95, 4: 0.95, 5: 0.95, 6: 0.95, 7: 0.93}
...
Analysis for T = fwd + bwd:
 ...
 'pipeline_with_non_parallel_comm': {0: 585.05,
                                     1: 702.43,
                                     2: 720.49,
                                     3: 720.77,
                                     4: 720.36,
...
forward {0: 0.72, 1: 0.88, 2: 0.87, 3: 0.87, 4: 0.87, 5: 0.87, 6: 0.8, 7: 0.8}
backward {0: 0.77, 1: 0.95, 2: 0.95, 3: 0.95, 4: 0.95, 5: 0.95, 6: 0.86, 7: 0.86}

worstcase: bwd: 516.289 fwd: 206.947
expected_speedup_compared_to_seq_no_recomp_no_comm: 5.178
Expected speedup for 8 partitions is: 7.072
max cuda memory used 7.09GB
"""