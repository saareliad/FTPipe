"""AutoGenerated with:
python -m autopipe.partition t5 --model_name_or_path t5-3b --t5_task squad1 --lmhead --n_iter 10 --analysis_batch_size 4 --partitioning_batch_size 64 --precompute_masks --stateless_tied --lmhead --n_partitions 8 --max_seq_length 64 --answer_max_seq_length 4 --ct t5_3b_bb_wic --save_memory_mode --preset ftpipe --partitioning_method mpipe --dont_use_async_meta_alg --basic_blocks T5Block --dot
"""
import torch
...
# partition 8 {'inputs': {0, 7, 'decoder_attention_mask', 'inverted_encoder_attention_mask'}, 'outputs': {9, 10, 11, 12, 13, 14, 15}}
# partition 9 {'inputs': {8, 'decoder_attention_mask', 'inverted_encoder_attention_mask'}, 'outputs': {10}}
# partition 10 {'inputs': {8, 9, 'decoder_attention_mask', 'inverted_encoder_attention_mask'}, 'outputs': {11}}
# partition 11 {'inputs': {8, 10, 'decoder_attention_mask', 'inverted_encoder_attention_mask'}, 'outputs': {12}}
# partition 12 {'inputs': {8, 11, 'decoder_attention_mask', 'inverted_encoder_attention_mask'}, 'outputs': {13}}
# partition 13 {'inputs': {8, 12, 'decoder_attention_mask', 'inverted_encoder_attention_mask'}, 'outputs': {14}}
# partition 14 {'inputs': {8, 13, 'decoder_attention_mask', 'inverted_encoder_attention_mask'}, 'outputs': {15}}
# partition 15 {'inputs': {8, 'lm_labels', 'decoder_attention_mask', 14, 'inverted_encoder_attention_mask'}, 'outputs': {'output'}}
# model outputs {15}
...
def create_pipeline_configuration(DEBUG=False, batch_size=64):
    config = {
        'batch_dim': 0,
        'depth': 10000,
        'basic_blocks': (StatelessEmbedding,Linear,T5Block,Dropout,CrossEntropyLoss,T5LayerNorm),
        'model_inputs': {
            'attention_mask': {
...
class Partition8(nn.Module):
    ...
    def forward(self, *args):
        ...
        # T5ForConditionalGeneration/T5Stack[decoder]/T5Block[1] <=> self.l_4
        # T5ForConditionalGeneration/T5Stack[decoder]/T5Block[2] <=> self.l_5
...
unique_stages_on_same_gpu: [{0, 10}, {1, 9}, {2, 11}, {3, 12}, {4, 13}, {5, 14}, {8, 6}, {15, 7}]
"stage_to_device_map": [0, 1, 2, 3, 4, 5, 6, 7, 6, 1, 0, 2, 3, 4, 5, 7],
...
forward {0: 19.74, 1: 19.37, 2: 18.37, 3: 18.3, 4: 18.01, 5: 19.12, 6: 19.91, 7: 20.62}
backward {0: 45.14, 1: 42.45, 2: 39.8, 3: 39.07, 4: 38.03, 5: 39.82, 6: 40.14, 7: 34.21}
...
forward {0: 0.94, 1: 0.92, 2: 0.87, 3: 0.87, 4: 0.85, 5: 0.91, 6: 0.95, 7: 1.0}
backward {0: 1.0, 1: 0.93, 2: 0.87, 3: 0.86, 4: 0.83, 5: 0.87, 6: 0.88, 7: 0.75}

worstcase: bwd: 45.136 fwd: 20.616
Expected speedup for 8 partitions is: 7.096
Assuming bandwidth of 12 GBps between GPUs

communication volumes size of activations of each partition
0: input size:'1.25 MB', recieve_time:'0.10 ms', out:'4.46 MB', send time:'0.37 ms'
1: input size:'4.40 MB', recieve_time:'0.37 ms', out:'4.40 MB', send time:'0.37 ms'
...
5: input size:'4.40 MB', recieve_time:'0.37 ms', out:'4.40 MB', send time:'0.37 ms'
6: input size:'4.26 MB', recieve_time:'0.35 ms', out:'4.40 MB', send time:'0.37 ms'
7: input size:'4.40 MB', recieve_time:'0.37 ms', out:'1.05 MB', send time:'0.09 ms'

Compuatation Communication ratio (comp/(comp+comm)):
forward {0: 0.98, 1: 0.98, 2: 0.98, 3: 0.98, 4: 0.98, 5: 0.98, 6: 0.98, 7: 1.0} 
...
Analysis for T = fwd + bwd:
 ...
 'pipeline_with_non_parallel_comm': {0: 64.87,
                                     1: 61.82,
                                     2: 58.17,
                                     3: 57.37,
                                     4: 56.04,
...
expected_speedup_compared_to_seq_no_recomp_no_comm: 5.794
Analysis max cuda memory used 1.88GB
"""