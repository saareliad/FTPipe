...
class T5Attention(nn.Module):
    ...
    def forward(
        ...
        """
        Self-attention (if is_None(key_value_states)) or attention over source sentence (provided by key_value_states).
        """
        ...
        if is_not_None(past_key_value):
            assert (
                len(past_key_value) == 2
            ), f"past_key_value should have 2 past states: keys and values. Got { len(past_key_value)} past states"
            real_seq_length += past_key_value[0].shape[2] if is_None(query_length) else query_length

        key_length = real_seq_length if is_None(key_value_states) else key_value_states.shape[1]
        ...
        def project(hidden_states, proj_layer, key_value_states, past_key_value):
            ...
            if is_None(key_value_states):
                # self-attn
                # (batch_size, n_heads, seq_length, dim_per_head)
                hidden_states = shape(proj_layer(hidden_states))
            elif is_None(past_key_value):
                # cross-attn
                # (batch_size, n_heads, seq_length, dim_per_head)
        ...
        key_states = project(
            hidden_states, self.k, key_value_states, past_key_value[0] if is_not_None(past_key_value) else None
        )
        value_states = project(
            hidden_states, self.v, key_value_states, past_key_value[1] if is_not_None(past_key_value) else None
        )
...
class T5Block(nn.Module):
    ...
    def forward(
        ...
        if is_not_None(past_key_value):
            ...
            expected_num_past_key_values = 2 if is_None(encoder_hidden_states) else 4
        ...
        do_cross_attention = self.is_decoder and is_not_None(encoder_hidden_states)
...
class T5PreTrainedModel(PreTrainedModel):
    ...
    def _init_weights(self, module):
        ...
        if isinstance(module, T5LayerNorm):
            ...
            if hasattr(module.wi, "bias") and is_not_None(module.wi.bias):
                module.wi.bias.data.zero_()
            module.wo.weight.data.normal_(mean=0.0, std=factor * ((self.config.d_ff) ** -0.5))
            if hasattr(module.wo, "bias") and is_not_None(module.wo.bias):
                module.wo.bias.data.zero_()
        elif isinstance(module, T5DenseGatedGeluDense):
            module.wi_0.weight.data.normal_(mean=0.0, std=factor * ((self.config.d_model) ** -0.5))
            if hasattr(module.wi_0, "bias") and is_not_None(module.wi_0.bias):
                module.wi_0.bias.data.zero_()
            module.wi_1.weight.data.normal_(mean=0.0, std=factor * ((self.config.d_model) ** -0.5))
            if hasattr(module.wi_1, "bias") and is_not_None(module.wi_1.bias):
...
class T5Stack(T5PreTrainedModel):
    ...
    def parallelize(self, device_map=None):
        ...
        self.device_map = (
            get_device_map(len(self.block), range(torch.cuda.device_count())) if is_None(device_map) else device_map
        )
    ...
    def forward(
        ...
        for i, (layer_module, past_key_value) in enumerate(zip(self.block, past_key_values)):
            ...
            if self.model_parallel:
                ...
                if is_not_None(position_bias):
                    position_bias = position_bias.to(hidden_states.device)
                if is_not_None(encoder_hidden_states):
                    encoder_hidden_states = encoder_hidden_states.to(hidden_states.device)
                if is_not_None(encoder_extended_attention_mask):
                    encoder_extended_attention_mask = encoder_extended_attention_mask.to(hidden_states.device)
                if is_not_None(encoder_decoder_position_bias):
                    encoder_decoder_position_bias = encoder_decoder_position_bias.to(hidden_states.device)
                if is_not_None(layer_head_mask):
                    layer_head_mask = layer_head_mask.to(hidden_states.device)
                if is_not_None(encoder_layer_head_mask):
...
class T5Model(T5PreTrainedModel):
    ...
    def forward(
        ...
        if is_not_None(head_mask) and is_None(decoder_head_mask):
        ...
        if self.model_parallel:
            ...
            if is_not_None(decoder_attention_mask):
...
class T5ForConditionalGeneration(T5PreTrainedModel):
    ...
    def forward(
        ...
        if is_not_None(labels) and is_None(decoder_input_ids) and is_None(decoder_inputs_embeds):
            # get decoder inputs from shifting lm labels to the right
            decoder_input_ids = self._shift_right(labels)

        # If decoding with past key value states, only the last tokens
        # should be given as an input
        if is_not_None(past_key_values):
            assert is_None(labels), "Decoder should not use cached key value states when training."
            if is_not_None(decoder_input_ids):
