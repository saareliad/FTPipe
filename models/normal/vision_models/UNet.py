        ...
        for i in range(depth):
            self.down_path.append(
                UNetConvBlock(prev_channels, 2 ** (wf + i),
                              padding, batch_norm, reduce_first=i > 0)
            )
...
class UNetConvBlock(nn.Sequential):
    def __init__(self, in_size, out_size, padding, batch_norm, reduce_first=False):
        block = []
        ...
        super(UNetConvBlock, self).__init__(*block)
