...
class PreTrainedModel(nn.Module, ModuleUtilsMixin, GenerationMixin):
    ...
    def tie_weights(self):
        ...
        if is_not_None(output_embeddings) and self.config.tie_word_embeddings:
    ...
    def _tie_or_clone_weights(self, output_embeddings, input_embeddings):
        ...
        if getattr(output_embeddings, "bias", is_not_None(None)):
    ...
    def _resize_token_embeddings(self, new_num_tokens):
        ...
        if is_not_None(self.get_output_embeddings()) and not self.config.tie_word_embeddings:
    ...
    def _get_resized_lm_head(
        ...
        has_new_lm_head_bias = is_not_None(old_lm_head.bias)
    ...
    def save_pretrained(self, save_directory: Union[str, os.PathLike]):
        ...
        if is_not_None(self._keys_to_ignore_on_save):
    ...
    def from_pretrained(cls, pretrained_model_name_or_path: Optional[Union[str, os.PathLike]], *model_args, **kwargs):
        ...
        if not isinstance(config, PretrainedConfig):
            config_path = config if is_not_None(config) else pretrained_model_name_or_path
        ...
        # Load model
        if is_not_None(pretrained_model_name_or_path):
        ...
        if is_None(state_dict) and not from_tf:
            try:
        ...
        if from_tf:
            ...
            if is_not_None(metadata):
                state_dict._metadata = metadata

            # PyTorch's `_load_from_state_dict` does not copy parameters in a module's descendants
            # so we need to apply the function recursively.
            def load(module: nn.Module, prefix=""):
                local_metadata = {} if is_None(metadata) else metadata.get(prefix[:-1], {})
            ...
            # the user.
            if is_not_None(cls._keys_to_ignore_on_load_missing):
                for pat in cls._keys_to_ignore_on_load_missing:
                    missing_keys = [k for k in missing_keys if is_None(re.search(pat, k))]

            if is_not_None(cls._keys_to_ignore_on_load_unexpected):
                for pat in cls._keys_to_ignore_on_load_unexpected:
                    unexpected_keys = [k for k in unexpected_keys if is_None(re.search(pat, k))]
...
class PoolerEndLogits(nn.Module):
    ...
    def forward(
        ...
        """
        assert (
            is_not_None(start_states) or is_not_None(start_positions)
        ), "One of start_states, start_positions should be not None"
        if is_not_None(start_positions):
...
class SQuADHead(nn.Module):
    ...
    def forward(
        ...
        if is_not_None(start_positions) and is_not_None(end_positions):
            ...
            if is_not_None(cls_index) and is_not_None(is_impossible):
            ...
            p_mask = p_mask.unsqueeze(-1) if is_not_None(p_mask) else None
...
def prune_linear_layer(layer: torch.nn.Linear, index: torch.LongTensor, dim: int = 0) -> torch.nn.Linear:
    ...
    new_layer = nn.Linear(new_size[1], new_size[0], bias=is_not_None(layer.bias)).to(layer.weight.device)
...
def prune_layer(
    ...
    if isinstance(layer, nn.Linear):
        return prune_linear_layer(layer, index, dim=0 if is_None(dim) else dim)
    elif isinstance(layer, Conv1D):
        return prune_conv1d_layer(layer, index, dim=1 if is_None(dim) else dim)
    else:
