...
class _RelativePositionBucket(nn.Module):
    def forward(self, relative_position, bidirectional=True, num_buckets=32, max_distance=128):
        """
...
class ComputeBias(nn.Module):
    def __init__(self):
        super(ComputeBias, self).__init__()
        self._relative_position_bucket = _RelativePositionBucket()
        ...
        if position_bias is None:
            ...
            if mask is not None:
                position_bias = position_bias + \
                    mask  # (bs, n_heads, qlen, klen)

        scores += position_bias
        # (bs, n_heads, qlen, klen)
        weights = self.softmax(scores.float()).type_as(scores)
        # (bs, n_heads, qlen, klen)
...
class T5PreTrainedModel(PreTrainedModel):
    ...
    def _shift_right(self, input_ids):
        ...
        ), "Verify that `lm_labels` has only positive values and -100"
    ...
    def from_pretrained(cls, pretrained_model_name_or_path, *model_args, **kwargs):
        ...
        Examples::
            ...
            config = BertConfig.from_json_file(
                './tf_model/my_tf_model_config.json')
            model = BertModel.from_pretrained(
                './tf_model/my_tf_checkpoint.ckpt.index', from_tf=True, config=config)
        ...
        if from_tf:
            ...
            for key in state_dict.keys():
                new_key = key
                # the original T5stack uses moduleList we do not
                if 'block.' in new_key:
    ...
    def forward(
        ...
        if attention_mask is None:
            attention_mask = torch.ones(
                batch_size, seq_length).to(inputs_embeds.device)
        if self.is_decoder and encoder_attention_mask is None:
            encoder_seq_length = encoder_hidden_states.shape[1]
            encoder_attention_mask = torch.ones(
                batch_size, encoder_seq_length).to(inputs_embeds.device)
        ...
        if head_mask is not None:
            if head_mask.dim() == 1:
                ...
                head_mask = head_mask.expand(
                    self.config.num_layers, -1, -1, -1, -1)
...
class T5Model(T5PreTrainedModel):
    ...
    def _prune_heads(self, heads_to_prune):
        ...
        for layer, heads in heads_to_prune.items():
            getattr(self.encoder, str(layer)).attention.prune_heads(heads)
    ...
    Examples::
            ...
            input_ids = tokenizer.encode(
                "Hello, my dog is cute", return_tensors="pt")  # Batch size 1
...
class T5ForConditionalGeneration(T5PreTrainedModel):
    ...
    Examples::
        ...
        model = T5ForConditionalGeneration.from_pretrained('t5-small')
        input_ids = tokenizer.encode(
            "summarize: Hello, my dog is cute", return_tensors="pt")  # Batch size 1
